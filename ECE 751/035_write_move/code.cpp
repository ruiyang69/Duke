...
class Header {
 public:
  ...
  Header(const Header & toCopy) : length(0),src(),dest(){
    length = toCopy.length;
    src = IPAddress(toCopy.src);
    dest = IPAddress(toCopy.dest);
  }

  // TODO: Write move constructor
  Header(Header && toMove) : length(0),src(),dest(){
    std::swap(length, toMove.length);
...
class Packet {
 public:
  ...
  // TODO: write copy constructor
  Packet(const Packet & toCopy) : len(0),hdr(),data(new unsigned char[toCopy.len]){
    len = toCopy.len;
    hdr = Header(toCopy.hdr);
    for(size_t i=0; i<len; i++) data[i] = toCopy.data[i]; 
  }

  // TODO: write move constructor
  Packet(Packet && toMove) : len(0),hdr(),data(nullptr){
      std::swap(len, toMove.len);    
      std::swap(hdr, toMove.hdr);   
